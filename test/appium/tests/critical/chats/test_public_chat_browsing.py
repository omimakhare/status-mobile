...
class TestCommunityMultipleDeviceMergedTwo(MultipleSharedDeviceTestCase):
    ...
    def test_community_join_when_node_owner_offline(self):
        ...
        if self.community_2.toast_content_element.is_element_displayed(10):
            if self.community_2.toast_content_element.text != exp_text:
                self.errors.append("Text \"%s\" is not shown in toast element" % exp_text)
        else:
        ...
        if not self.home_2.get_chat(community_name, community=True).is_element_displayed():
            self.errors.append("%s chat is not listed inside Pending communities tab" % community_name)
        ...
        if chat_element.is_element_displayed(30):
            ...
            self.errors.append("%s chat is not listed inside Joined communities tab" % community_name)
